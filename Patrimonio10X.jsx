...
function project(years, monthly, ratePct, initial) {
  ...
  for (let y = 0; y <= years; y++) {
    pts.push({ year: 2026 + y, value: Math.round(bal), aportado: Math.round(initial + monthly * 12 * y) });
...
export default function App() {
  ...
  const C = tokens(dark);
  const [booted, setBooted] = useState(false);      // onboarding concluído?
  const [tab, setTab] = useState("inicio");
  const [lesson, setLesson] = useState(null);

  const [user] = useState({
    nome: "Você", instituicao: "Nubank", aporte: 2000, risco: "moderado",
    conhecimento: "iniciante", invested: 16000, patrimonio: 17240, meses: 8,
  });
  const lucro = user.patrimonio - user.invested;
  const rentab = (lucro / user.invested) * 100;
  ...
  if (!booted) return <Onboarding C={C} dark={dark} setDark={setDark} onDone={() => setBooted(true)} />;
  ...
  return (
    <div style={shell}>
      <Header C={C} dark={dark} setDark={setDark} nivel={nivel} xpNivel={xpNivel} streak={user.meses} />

      <div style={{ padding: "0 18px" }}>
        {tab === "inicio" && <Dashboard C={C} user={user} lucro={lucro} rentab={rentab} goals={goals} items={items} />}
        {tab === "metas" && <Goals C={C} goals={goals} patrimonio={user.patrimonio} aporte={user.aporte} />}
        {tab === "mentor" && <Mentor C={C} user={user} lucro={lucro} />}
        {tab === "aprender" && <Learn C={C} onOpen={setLesson} />}
        {tab === "calc" && <Calc C={C} />}
      </div>

      {tab === "inicio" && <MonthlyCard C={C} items={items} setItems={setItems} allDone={allDone} xp={xp} setXp={setXp} />}
...
function Header({ C, dark, setDark, nivel, xpNivel, streak }) {
  return (
...
/*  Onboarding                                                         */
/* ------------------------------------------------------------------ */
function Onboarding({ C, dark, setDark, onDone }) {
    ...
    return (
      <div style={{ ...wrap, alignItems: "center", justifyContent: "center", textAlign: "center", gap: 18 }}>
        ...
        <div style={{ color: C.textMut, fontSize: 14, maxWidth: 280 }}>
          A IA está criando sua reserva de emergência, suas metas e a distribuição mensal dos aportes.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={wrap}>
      ...
      <button onClick={() => {
        if (isLast) { setGenerating(true); setTimeout(onDone, 1900); }
        else setStep(step + 1);
...
/*  Dashboard                                                          */
/* ------------------------------------------------------------------ */
function Dashboard({ C, user, lucro, rentab, goals, items }) {
  const history = [
  ...
  const pct = Math.min(100, ((user.patrimonio - prevGoalV) / (nextGoal.v - prevGoalV)) * 100);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: C.hero, borderRadius: 26, padding: 22, color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: 18, marginTop: 18 }}>
          ...
          <MiniStat l="Este mês" v={brl(user.aporte)} />
          <MiniStat l="Investindo há" v={`${user.meses} meses`} />
        </div>
      </div>
      ...
      <Card C={C}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 12.5, color: C.textMut }}>
          ...
          <span>faltam {brl(nextGoal.v - user.patrimonio)}</span>
        </div>
      </Card>
...
/* ------------------------------------------------------------------ */
function MonthlyCard({ C, items, setItems, allDone, xp, setXp }) {
  const [celebrated, setCelebrated] = useState(false);
  useEffect(() => { if (allDone && !celebrated) { setCelebrated(true); setXp(x => x + 120); } }, [allDone]);
  ...
  return (
    <div style={{ padding: "14px 18px 0" }}>
      <Card C={C} style={{ background: allDone ? C.primarySoft : C.surface, borderColor: allDone ? C.primary : C.border }}>
        {allDone ? (
          <div style={{ textAlign: "center", padding: "6px 0" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 17 }}>Aportes de agosto concluídos! 🎉</div>
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              ...
              <span style={{ fontWeight: 800, fontSize: 15.5 }}>Plano de agosto</span>
            </div>
            <div style={{ color: C.textMut, fontSize: 13, marginBottom: 14 }}>
              Você recebeu o salário — vamos investir {brl(total)}. Marque conforme concluir cada um.
            </div>
            ...
            <div style={{ marginTop: 12, height: 6, borderRadius: 6, background: C.surfaceAlt, overflow: "hidden" }}>
              <div style={{ width: `${(doneVal / total) * 100}%`, height: "100%", background: C.primary, transition: "width .3s" }} />
            </div>
          </>
        )}
      </Card>
                ...
                {current && (
                  <>
                    ...
                    <div style={{ fontSize: 12, color: C.textMut }}>
                      Faltam {brl(falta)} · ~{meses} {meses === 1 ? "mês" : "meses"} no ritmo atual
                    </div>
                  </>
                )}
...
/* ------------------------------------------------------------------ */
/*  Mentor IA — chat real com fallback educacional                    */
/* ------------------------------------------------------------------ */
const FALLBACK = [
...
function Mentor({ C, user, lucro }) {
  const suggestions = ["O que é ETF?", "O mercado caiu, devo vender?", "O que são juros compostos?", "Vale a pena comprar dólar?"];
  const [msgs, setMsgs] = useState([
    { role: "assistant", text: `Oi! Sou seu mentor. Já vi que você tem ${brl(user.patrimonio)} investidos e vem mantendo o ritmo há ${user.meses} meses — parabéns pela disciplina. Pode me perguntar qualquer coisa, sem medo de parecer básico.` },
  ]);
  ...
  async function send(text) {
    ...
    const system = `Você é o Mentor IA do app Patrimônio 10X, um mentor financeiro para iniciantes brasileiros.
Contexto do usuário: patrimônio ${brl(user.patrimonio)}, aporte mensal ${brl(user.aporte)}, perfil de risco ${user.risco}, conhecimento ${user.conhecimento}, instituição ${user.instituicao}, lucro acumulado ${brl(lucro)}.
Regras invioláveis:
...
- Deixe claro, quando der conselho, que é conteúdo educacional e não recomendação personalizada.
Responda em português do Brasil, com tom acolhedor.`;
